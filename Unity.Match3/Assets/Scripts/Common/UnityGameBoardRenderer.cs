...
namespace Common
{
    public class UnityGameBoardRenderer : MonoBehaviour, IUnityGameBoardRenderer, IGameBoardDataProvider<IUnityGridSlot>
    {
        ...
        [SerializeField] private int _turns = 15;
        [SerializeField] private int _levelGoal = 100;
        ...
        public IUnityGridSlot[,] GameBoardSlots => _gameBoardSlots;
        public IGridTile[,] GridSlotTiles => _gridSlotTiles;

        private void Awake()
        {
            _tileItemsPool = new TileItemsPool(_gridTiles, transform);
            MovesCountController.SetTurns(_turns);
            LevelGoalController.SetLevelGoal(_levelGoal, _itemID);
        }

...
namespace Match3.App
{
    public abstract class Match3Game<TGridSlot> : BaseGame<TGridSlot> where TGridSlot : IGridSlot
    {
        ...
        public bool IsSwapItemsCompleted
        {
        ...
        public async UniTask StartAsync(CancellationToken cancellationToken = default)
        {
            ...
            await CheckAllItems(_fillStrategy);
        ...
        public UniTask SwapItemsAsync_c(GridPosition position1, GridPosition position2, CancellationToken cancellationToken = default)
        {
            return SwapItemsAsync(position1, position2, cancellationToken).ToAsyncLazy().Task;
        }
        ...
        public UniTask CheckAll_c(CancellationToken cancellationToken = default)
        {
            return CheckAll();
        }

        protected UniTask CheckAll(CancellationToken cancellationToken = default)
        {
            if (_swapItemsTask?.Task.Status.IsCompleted() ?? true)
            {
                _swapItemsTask = CheckAllItems(_fillStrategy, cancellationToken).ToAsyncLazy();
            }
            ...
            GridPosition position2, CancellationToken cancellationToken = default)
        {
            ...
            if (IsSolved(position1, position2, out var solvedData))
            {
                ...
                await CheckAllItems(fillStrategy);
            }
            else
            {
                await SwapGameBoardItemsAsync(position1, position2, cancellationToken);
        ...
        public virtual async UniTask CheckAllItems(IBoardFillStrategy<TGridSlot> fillStrategy, CancellationToken cancellationToken = default)
        {
            while (IsSolvedAny(out var solvedData))
            {
                NotifySequencesSolved(solvedData);
                await ExecuteJobsAsync(fillStrategy.GetSolveJobs(GameBoard, solvedData), cancellationToken);
            }
        ...
        public bool IsSolvedAny(out SolvedData<TGridSlot> solvedData)
        {
            solvedData = _gameBoardSolver.SolveAll(GameBoard);
